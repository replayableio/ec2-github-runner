...
async function startRunnerCommand(instanceId) {
  ...
  const commands = [
    "Start-Transcript -Path 'C:\\RunnerLog.txt'",
    "cd C:\\actions-runner",
    `./config.cmd --token ${token} --url https://github.com/replayableio/testdriver --labels ${instanceId} --name ${instanceId} --unattended --ephemeral --runasservice --windowslogonaccount testdriver --windowslogonpassword wwv9uJ0sqlulbN3`
  ];
